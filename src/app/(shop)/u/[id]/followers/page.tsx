...
/**
 * Lists the users who follow `id` (rows where followee_id = id), joined to
 * their profiles. Each row links to that user's own /u/[id] page.
 */
export default async function FollowersPage({ params }: Props) {
  ...
  const [{ data: profile }, { data: rows }] = await Promise.all([
    ...
    supabase
      .from("follows")
      .select("follower:follower_id(id, display_name, is_verified)")
      .eq("followee_id", id)
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8">
      <h1 className="text-2xl font-semibold text-slate-900">
        People following {profile.display_name ?? "user"}
      </h1>
      <p className="text-sm text-slate-500 mb-6">{users.length} followers</p>
